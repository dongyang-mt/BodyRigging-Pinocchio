...
{
    int i;
#define OUT { vertices.clear(); edges.clear(); textures.clear(); return; }
...
void Mesh::readFbx(const string &file, const string &hint) {
    parseFbx(*this, file, hint);
}
...
void Mesh::computeVertexTextures()
{
    int i;
    for(i = 0; i < (int)vertices.size(); ++i)
        vertices[i].texture = Pinocchio::Vector3();
    ...
    for(i = 0; i < (int)edges.size(); i += 3) {
        ...
        Pinocchio::Vector3 texture = ((vertices[i2].pos - vertices[i1].pos) % (vertices[i3].pos - vertices[i1].pos)).normalize();
        vertices[i1].texture += texture;
        vertices[i2].texture += texture;
        vertices[i3].texture += texture;
    }

    for(i = 0; i < (int)vertices.size(); ++i)
        vertices[i].texture = vertices[i].texture.normalize();
}
...
void Mesh::computeVertexNormals()
{
    int i;
    for(i = 0; i < (int)vertices.size(); ++i)
        vertices[i].normal = Pinocchio::Vector3();
    for(i = 0; i < (int)edges.size(); i += 3) {
        int i1 = edges[i].vertex;
        int i2 = edges[i + 1].vertex;
        int i3 = edges[i + 2].vertex;
        Pinocchio::Vector3 normal = ((vertices[i2].pos - vertices[i1].pos) % (vertices[i3].pos - vertices[i1].pos)).normalize();
...
void Mesh::normalizeBoundingBox()
{
    ...
    Pinocchio::Vector3 ctoAdd = Pinocchio::Vector3(0.5, 0.5, 0.5) - boundingBox.getCenter() * cscale;
    for(i = 0; i < (int)vertices.size(); ++i) {
...
void Mesh::readObj(istream &strm)
{
    ...
    while(!strm.eof()) {
        ...
        if(words[0][0] == 'v') {
            ...
            vertices.back().pos = Pinocchio::Vector3(x, y, z);
        }
        ...
        //     textures.back().pos = Pinocchio::Vector3(x, y);
...
void Mesh::readPly(istream &strm)
{
    ...
    while(!strm.eof()) {
        ...
        if(vertsLeft > 0) {
            ...
            vertices.resize(vertices.size() + 1);
            vertices.back().pos = Pinocchio::Vector3(-z, x, -y);
            continue;
        }
...
void Mesh::readGts(istream &strm)
{
    ...
    while(!strm.eof()) {
        ...
        if(vertsLeft > 0) {
            ...
            vertices.resize(vertices.size() + 1);
            vertices.back().pos = Pinocchio::Vector3(-x, z, y);
...
class StlVtx : public Pinocchio::Vector3
{
public:
    StlVtx(double x, double y, double z) : Pinocchio::Vector3(x, y, z) {}
